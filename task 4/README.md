...
This project demonstrates core API fundamentals:
REST concepts
- HTTP methods (GET, POST, PUT, DELETE)
...
REST: Representational State Transfer – designing APIs using resources and standard HTTP methods.
HTTP Methods:
GET → read data
POST → create data
PUT → update data
DELETE → remove data
Flask:
@app.route() → defines routes/endpoints
request.get_json() → read JSON
jsonify() → return JSON responses
In-memory storage:
Using a Python dictionary instead of a real database (for learning purposes).